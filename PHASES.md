...
### Second gotcha — remediating what the scan found

First green-to-red run: **7 HIGH, 0 CRITICAL**, all with fixes available. All
seven were Python packages, not OS packages. They split into two groups with
genuinely different correct answers.

**Group 1 — real dependencies. Upgrade them.**

`starlette 0.41.3`, the ASGI layer FastAPI runs on, so this is code in the
live request path:
...
| CVE-2026-54283 | `request.form()` size limits silently ignored |

Fixed by moving to `starlette 1.6.0`, which pulled `fastapi` to `0.141.1`.
starlette is now pinned **directly** in requirements.txt even though fastapi
depends on it — a security-relevant dependency should be visible to a
reviewer, not an implicit consequence of another pin.

**Group 2 — things that should not be in the image at all. Delete them.**

`wheel` (privilege escalation) and `jaraco.context` (path traversal, vendored
inside setuptools) are not dependencies of this service. Nothing imports them.
They were present because `python -m venv` seeds every environment with pip,
setuptools, and wheel automatically.

> **The most durable way to fix a vulnerability in a component is to not ship
> the component.** Upgrading treats a symptom you will treat again next
> quarter. Removing ends it. It also removes real risk: if someone gets code
> execution in your container, `pip` is a ready-made tool for pulling in more.

**Then the fix failed, identically, and that was the useful part.**

Second run: still **2 HIGH** — the same two packages, at byte-for-byte
identical versions. When a fix changes *nothing at all* about a finding, you
didn't fix the wrong thing; you fixed a different copy of the right thing.

There are two copies. The builder-stage cleanup handled the virtualenv. The
base image ships a **second** set in the system site-packages at
`/usr/local/lib/python3.11/site-packages`, untouched. Removing both cleared it.

The path is resolved at build time rather than hardcoded, so bumping the base
image to 3.12 later cannot silently turn the cleanup into a no-op.

### Verify

Open a PR. Both checks green. Confirm the "Verify build provenance" step
prints matching reported/expected SHAs.

**Status: green.** Lint and test ~19s, build and scan ~58s.
...
| File | Purpose |
|---|---|
| `scripts/setup-gcp.sh` | One-time, idempotent GCP setup |
| `.github/workflows/deploy.yml` | Build → push → deploy → verify production |

### Why

**No service account key exists anywhere.** The tutorial approach — download a
service account JSON key, paste it into a GitHub secret — creates a
credential that never expires, works from anywhere, and ends up in your shell
history and a backup. Leaked service account keys are one of the most common
causes of cloud compromise, and the access looks legitimate in the logs.

Workload Identity Federation replaces it. GitHub Actions requests a
short-lived OIDC token asserting *"I am repository OWNER/REPO"*. Google
validates it against a trust policy and issues an access token good for
minutes. Requires `id-token: write` in the workflow permissions.

**The attribute condition is the security boundary.** Without
`assertion.repository_owner == 'you'`, the policy trusts any token GitHub
Actions issues — and it issues them to every repo on GitHub, including one an
attacker makes in thirty seconds. Google now requires a condition because this
was a widespread real misconfiguration.

**Two service accounts.** The deployer (push images, deploy) and the runtime
identity the service actually runs as (no roles at all). Cloud Run defaults to
the Compute Engine SA, which holds project Editor — so a compromised request
would run with broad write access to the whole project.

**Tag images with the commit SHA, not `:latest`.** `:latest` means "whatever
was pushed most recently", which is useless mid-incident and impossible to
roll back to deterministically. Rollback becomes: deploy an earlier SHA tag.

**Separate workflow from CI.** ci.yml runs on pull requests from anywhere and
must never hold deploy credentials. `id-token: write` appears only here.

### Gotchas hit — four of them

**1. IAM is eventually consistent.** `gcloud iam service-accounts create`
returns success before IAM will accept a binding referencing the new account:

```
ERROR: INVALID_ARGUMENT: Service account ... does not exist.
```

The account existed. IAM hadn't caught up. Every binding is now wrapped in a
retry with backoff. *Infrastructure code needs retries where application code
never would.*

**2. Missing buildx setup.** `cache-to: type=gha` requires the
docker-container driver. Without `docker/setup-buildx-action` the default
`docker` driver is used and the build dies with "Cache export is not supported
for the docker driver." ci.yml had the step; deploy.yml didn't. *Same
Dockerfile, green in CI, red in deploy — the difference was runner setup, not
code.*

**3. Cloud Run reserves URL paths ending in "z".** `/healthz` and `/readyz` are
404'd by Google's frontend before reaching the container. The symptom set is
genuinely disorienting:

- `gcloud run deploy` succeeds, revision reports healthy
- `/version` returns your app's real JSON
- `/healthz` returns a **Google-branded** 404
- nothing in the application logs — the request never arrived

> **The tell: the 404 page didn't look like FastAPI's.** Two different error
> pages means two different servers answered. When an error page doesn't match
> your framework's, something upstream is answering for you.

Primary paths are now `/health` and `/ready`; the `z` variants stay as aliases
since they are the Kubernetes convention and work locally, in Docker, and on
GKE.

**4. A smoke test that couldn't explain itself.** The original polled with
`curl -sf` and discarded the response, so "alive but 404" was
indistinguishable from "dead". It burned 60s and reported "never became
healthy" about a perfectly healthy service. It now prints the HTTP status each
attempt with a legend: `000` connect/DNS, `403` not publicly invokable, `404`
reached Google but not routed to the container.

> **A health check that cannot tell you why it failed is barely a health
> check.**
...
curl -s "$URL/version"    # commit must equal `git rev-parse HEAD`
```

**Status: live.** Production reported `56c8788`, matching HEAD exactly — the
full chain from keyboard to public URL, confirmed by asking the running
...
| Scan: 7 HIGH in Python packages | Outdated starlette + build tooling shipped in the image | Upgrade the real deps; delete the build tooling |
| Same 2 findings after "fixing" them | Second copy in system site-packages, not just the venv | Clean both locations |
| Download never arrived | File card previewed instead of downloaded | Click the download icon, or apply changes by hand |
| `Service account does not exist` right after creating it | IAM is eventually consistent | Retry with backoff |
| `Cache export is not supported for the docker driver` | Missing `setup-buildx-action` | Add it before the build step |
| Cloud Run 404s `/healthz`, app logs empty | Paths ending in "z" are reserved by Google's frontend | Rename to `/health`, `/ready` |
| Smoke test said "never became healthy" on a healthy service | `curl -sf` hides the status code | Report the HTTP code on every attempt |
| `! [rejected] main -> main (fetch first)` | Remote moved ahead of local | `git pull --rebase` — never `--force` on a shared branch |
...
- **If a fix changes nothing about a finding, you fixed a different copy.**
  Identical versions in the report after a remediation attempt means the thing
  you edited was not the thing being scanned.
- **Believe the scanner over your mental model of the image.** It reads what
  actually shipped; you are remembering what you meant to ship.
- **Chain dependent commands with `&&`.** Newline-separated commands keep
  running after a failure and every one of them will report success.
- **Read whose error page it is.** A 404 that doesn't look like your
  framework's 404 means something upstream answered instead of your app.
- **Cloud APIs are eventually consistent.** Create-then-immediately-reference
  fails with misleading "does not exist" errors. Retry before debugging.
- **When it works in one pipeline and fails in another with identical code,
  compare the environments, not the code.**
